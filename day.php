...
# check "visitor viewable range"
# if out, then redirect to another page
# if in, then continue
...
$currentMonth = date("m");
$currentYear  = date("Y");
$currentTS=mktime(0,0,0,$currentMonth,$currentDay,$currentYear);
$dateDistance=($currentTS-$ts)/3600/24;
if (($dateDistance>0) or ($dateDistance<-13)) { 
	$viewable=0; }
else { 
	$viewable=1; }

if(!isLoggedIn() and $viewable==0) {
	showAccessDeniedForNotLogin($day, $month, $year, $area, "day");
	exit();
}
...
else
{
	...
	echo "<table cellspacing=0 border=1 width=\"100%\" class=\"canlendar_day\">";
	echo "<tr><th width=\"1%\">".($enable_periods ? get_vocab("period") : get_vocab("time"))."</th>";		// marco - mid table time header

	$room_column_width = (int)(95 / sql_count($res));
	for ($i = 0; ($row = sql_row($res, $i)); $i++)		// marco - mid table room headers
	{
	...
	)
	{
		...
		if( $enable_periods ){
			...
			echo "<a href=\"$hilite_url=$time_t\" title=\"" // marco - mid table left side time cols
            . get_vocab("highlight_line") . "\">"
            . utf8_date(hour_min_format(),$t) . "-".utf8_date(hour_min_format(),$t+3600)."</a></td>\n";
		}
		...
		while (list($key, $room) = each($rooms))
		{
			...
			tdcell($c);		// marco - mid table other cols (start from 2nd)
	...
	show_colour_key();		// marco - colorDivision 1 (text in lang.utf8) �|�p�ǽҵ{, �ӷ~�ǽҵ{, ... table
}
