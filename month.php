...
if(!isLoggedIn()) {
	showAccessDeniedForNotLogin($day, $month, $year, $area, "weekmonth");
	exit();
}
...
for ($day_num = 1; $day_num<=$days_in_month; $day_num++) {
	...
	else for ($i = 0; ($row = sql_row($res, $i)); $i++)
	{
	    if ($debug_flag)
            ...
            $d[$day_num]["type"][] = $row[4];			// marco - mark for adding type row4
            ...
            switch ($monthly_view_entries_details)
            {
                case "description":
                {
                    echo "<a href=\"view_entry.php?id=" . $d[$cday]["id"][$i]
                        ...
                        ."[" .$d[$cday]["type"][$i]. "] " . $d[$cday]["shortdescrip"][$i]
                ...
                case "both":
                {
                    ...
                    ."[" .$d[$cday]["type"][$i]. "] " . $d[$cday]["shortdescrip"][$i] . "</a>";
