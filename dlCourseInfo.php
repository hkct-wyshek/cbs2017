	...
	if ($submit == 0){
		...
		if(($end_date < $start_date) ){
			...
			$sql = "SELECT start_time, end_time, name, e.description, area_name, room_name ". 
				   ...
				   "WHERE e.room_id = r.id AND a.id = r.area_id AND type='".getUserName()."' and e.start_time >= '$start_date' and e.end_time <= '$end_date' AND a.cpname IN('".implode("','", $cp_name)."')  ORDER BY e.start_time, e.end_time";
