...
if ((isAdmin()||isCourseAdmin())&&(getWritable($create_by,getUserName()))&&(($year>=2007)||isAdmin())) {
	if( ! $series )
		if (isCourseAdmin()){
			$edit_date = mktime(0,0,0, $month, $day, $year);
...
<?php
if ( isAdmin() || (getDeletable($create_by,getUserName(),$stime)) ) {
	if( ! $series )
