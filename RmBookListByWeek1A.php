...
$fid = md5(rand());
define("CSVFILE","CampusUsage/"."ClassRmListingByWeek(DAY)_".$fid.".xls");
...
$endtime = $starttime + 5*24*60*60 + 5*60*60;

$satDate = date("Y",$endtime)."-".date("m",$endtime)."-".date("d",$endtime);
...
$fsql = "select id,room_name from mrbs_room where location=\"".$campus."\" and sequence <> 0 order by sequence asc";
...
echo "<h4><FONT COLOR=RED><STRONG>完成製作 - ".$campusName." (".$monDate." MON - ".$satDate." SAT)"."</STRONG></FONT></h4>";
...
fwrite($fp, " <Worksheet ss:Name=\"(".$monDate." to ".$satDate.")"."\">\n");

fwrite($fp, "  <Names>\n");
fwrite($fp, "   <NamedRange ss:Name=\"Print_Titles\" ss:RefersTo=\"='".$campusName."(".$monDate." to ".$satDate.")"."'!R1:R5\"/>\n");
fwrite($fp, "  </Names>\n");
...
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle\"><Data ss:Type=\"String\">全日制課程每週課室表 (".$campusName.")</Data><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
...
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle_RED\"><Data ss:Type=\"String\">".$monDate." to ".$satDate."</Data><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle\"><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle\"><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle\"><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
fwrite($fp, "  <Cell ss:StyleID=\"HeaderTitle\"><NamedCell ss:Name=\"Print_Titles\"/></Cell>\n");
...
$x = $x + 24*60*60;
$strDate = date("d",$x)."-".date("M",$x)."-".date("Y",$x)." (".date("D",$x).")";
fwrite($fp, "  <Cell ss:MergeAcross=\"1\" ss:StyleID=\"CenterBoldFontWithBorderLine\"><Data ss:Type=\"String\">".$strDate."</Data><NamedCell
ss:Name=\"Print_Titles\"/></Cell>\n");
fwrite($fp, "  </Row>\n");
...
# start displaying lesson records below
...
#		echo strftime("%m-%d [%H:%M]",$lessonstart)."-".strftime("[%H:%M]",$lessonend)." ".$lessonname."<br>";
	...
	for ($c=0;$c<5;$c++) {
		...
		$loc = array();
		for ($i=0;$i<$r;$i++) {
			if ( $rmbook[$i]["lessonstart"]>=($t1) and $rmbook[$i]["lessonstart"]<($t1+3*60*60) ) {
				$numSuit++;
				$loc[$numSuit] = $i;
			}
		}
		...
		if ($numSuit==0) {
			...
			$y="";
			for ($x=1; $x<=$numSuit; $x++) {
				$y = $y.$rmbook[$loc[$x]]["lessonname"];
			}
			fwrite($fp, "  <Cell ss:MergeAcross=\"1\" ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$y."</Data></Cell>\n");
		...
		$loc = array();
		for ($i=0;$i<$r;$i++) {
			if ( $rmbook[$i]["lessonstart"]>=($t1+3*60*60) and $rmbook[$i]["lessonstart"]<($t1+10*60*60) ) {
				$numSuit++;
				$loc[$numSuit] = $i;
			}
		}
		...
		if ($numSuit==0) {
			...
			fwrite($fp, "  <Cell ss:MergeAcross=\"2\" ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$y."</Data></Cell>\n");

		} elseif ($numSuit==2) {

			if (($rmbook[$loc[1]]["lessonend"]-$rmbook[$loc[1]]["lessonstart"]) == ($rmbook[$loc[2]]["lessonend"]-$rmbook[$loc[2]]["lessonstart"])) {
				fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[1]]["lessonname"]."</Data></Cell>\n");
				fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[2]]["lessonname"]."</Data></Cell>\n");
				fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\"></Data></Cell>\n");
			} elseif (($rmbook[$loc[1]]["lessonend"]-$rmbook[$loc[1]]["lessonstart"]) > ($rmbook[$loc[2]]["lessonend"]-$rmbook[$loc[2]]["lessonstart"])) {
				fwrite($fp, "  <Cell ss:MergeAcross=\"1\" ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[1]]["lessonname"]."</Data></Cell>\n");
				fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[2]]["lessonname"]."</Data></Cell>\n");
			} else {
				fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[1]]["lessonname"]."</Data></Cell>\n");
				fwrite($fp, "  <Cell ss:MergeAcross=\"1\" ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[2]]["lessonname"]."</Data></Cell>\n");
			}
		
		} elseif ($numSuit==3) {
			fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[1]]["lessonname"]."</Data></Cell>\n");
			fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[2]]["lessonname"]."</Data></Cell>\n");
			fwrite($fp, "  <Cell ss:StyleID=\"ClassBox\"><Data ss:Type=\"String\">".$rmbook[$loc[3]]["lessonname"]."</Data></Cell>\n");
...
$url = "CampusUsage/ClassRmListingByWeek(DAY)_".$fid.".xls";
